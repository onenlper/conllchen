...
public class ILPUtil {
	...
	public static HashMap<String, Double> loadCorefSVMProb(ArrayList<EventMention> mentions, String fileID) {
		...
		for (int i = 0; i < pairLines.size(); i++) {
			...
			m1.headCharStart = Integer.valueOf(tokens[0]);
			m1.headCharEnd = Integer.valueOf(tokens[1]);

			m2.headCharStart = Integer.valueOf(tokens[2]);
			m2.headCharEnd = Integer.valueOf(tokens[3]);
...
//			System.err.println(confLine.toString() + "#" + confidence);
	...
	public static void loadSVMResult() {
		...
		ArrayList<String> lines = Common
				.getLines("/users/yzcchen/workspace/NAACL2013-B/src/data/Joint_triggers_test_system" + Common.part);
...
//					System.err.println(typeConfidence[f]);
			...
			em.headCharEnd = end;
			em.typeConfidences = typeConfidence;
