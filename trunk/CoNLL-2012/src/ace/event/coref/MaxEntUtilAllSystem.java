...
public class MaxEntUtilAllSystem {
	...
	private static ArrayList<EntityMention> getSieveCorefMentions(CoNLLPart part) {
		String baseFolder = "/users/yzcchen/chen3/conll12/chinese/ACE_test_" + Common.part + "/";
...
//				assignGoldSemantic(mention, part);
	...
	private static ArrayList<EntityMention> getGoldTestEventMentions(CoNLLPart part) {
		...
		if(Common.train) {
			...
			// time mentions
			timeMentions = ACECommon.getTimeExpression(part);
			// value mentions
			valueMentions = ACECommon.getValueExpression(part);
		}
