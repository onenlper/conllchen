...
public abstract class Sieve {
	...
	public boolean isRoleAppositive(EntityMention can, EntityMention cur) {
		...
		if (can.headCharEnd + 1 == cur.headCharStart && cur.ner.equalsIgnoreCase("PERSON")) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isCopular(EntityMention can, EntityMention cur, CoNLLPart part) {
		...
		int position1[] = ChCommon.getPosition(can, part.getCoNLLSentences());
		int position2[] = ChCommon.getPosition(cur, part.getCoNLLSentences());
	...
	public boolean haveIncompatibleModify(EntityMention antecedent, EntityMention mention, CoNLLPart part) {
		for (EntityMention ant : antecedent.entity.mentions) {
			for (EntityMention em : mention.entity.mentions) {
				...
				Set<String> thisWordSet = new HashSet<String>(mention.modifyList);
				Set<String> antWordSet = new HashSet<String>(antecedent.modifyList);
