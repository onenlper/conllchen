...
public class ZeroCorefTestTkEvaluate extends ZeroCoref {

	String folder;
	SVMLightModel model;

	public ZeroCorefTestTkEvaluate(String folder) {
		this.folder = folder;
		...
		try {
			this.model = SVMLightModel.readSVMLightModelFromURL(new java.io.File(
					"/users/yzcchen/tool/JNI_SVM-light-6.01/src/svmlight-6.01/zeroCorefModel." + f).toURL());
...
//							System.out.print(coref + " ");
					...
					String key = part.getPartName() + " " + cand.toName() + " " + zero.toName();
					double d = 0;
					if(probKeys.containsKey(key)) {
						d = probKeys.get(key);
					}
...
//				removes.add(zero);
			...
			if (zero.antecedent != null
					...
					&& chainMap.get(zero.toName()).intValue() == chainMap.get(zero.antecedent.toName())
							.intValue()) {
				good++;
				// System.out.println("+++");
...
//			String np = fea.getObjectNP(zero);
	...
	static HashMap<String, Double> probKeys = new HashMap<String, Double>();
	...
	public static void main(String args[]) {
		...
		ArrayList<String> probs = Common.getLines("tk.prob2");
		ArrayList<String> keys = Common.getLines("mzeros.all");
		assert probs.size() == keys.size();
		for (int i = 0; i < probs.size(); i++) {
			double prob = Double.parseDouble(probs.get(i));
			String key = keys.get(i);
			probKeys.put(key, prob);
		}
		ZeroCorefTestTkEvaluate test = new ZeroCorefTestTkEvaluate(args[0]);
