		...
		for (EntityMention zero : anaphorZeros) {
			...
			for (int h = candidates.size() - 1; h >= 0; h--) {
				...
				if (cand.compareTo(zero) < 0 && cand.end < zero.start) {
					...
					feaStr = "-1 " + feaStr;
					double d = model.classify(Common.SVMStringToFeature(feaStr));
			...
			if (zero.antecedent != null && zero.antecedent.end != -1 && chainMap.containsKey(zero.toName())
				...
				String key = part.getDocument().getDocumentID() + ":" + part.getPartID() + ":" + zero.start + "-" + zero.antecedent.start
						+ "," + zero.antecedent.end + ":GOOD";
				corrects.add(key);
			} else {

				String key = part.getDocument().getDocumentID() + ":" + part.getPartID() + ":"
