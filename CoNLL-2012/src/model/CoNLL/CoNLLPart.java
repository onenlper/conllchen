...
public class CoNLLPart {
	...
	public CoNLLWord getWord(int wordIdx) {
		for (int i = 0; i < sentences.size(); i++) {
			...
			if (wordIdx >= sentence.getStartWordIdx() && wordIdx <= sentence.getEndWordIdx()) {
			...
			for (int i = 0; i < sentence.getWordsCount(); i++) {
				...
				for (String token : tokens) {
					...
					if (leftBracket != -1) {
						...
						int clusterID = Integer.valueOf(token.substring(leftBracket + 1, rightEnd));
					...
					if (rightBracket != -1) {
						...
						int clusterID = Integer.valueOf(token.substring(leftStart, rightBracket));
						for (int k = curMentions.size() - 1; k >= 0; k--) {
							...
							if (em.entityIndex == clusterID) {
								...
								for (int m = em.getStart(); m <= em.getEnd(); m++) {
									content.append(this.getWord(m).word).append(" ");
									orginal.append(this.getWord(m).orig).append(" ");
								}
								em.source = content.toString().trim();
								em.original = orginal.toString().trim();
			...
			for(int i=0;i<roles;i++) {
				...
				for(int k=0;k<s.words.size();k++) {
					...
					if(label.startsWith("(V*")) {
						EventMention m = new EventMention();
			...
			for (int index = 0; index < this.wordCount; index++) {
				...
				for (int s = index; s < this.wordCount; s++) {
					...
					if (word.equals("\"") || word.equals("``") || word.equals("“") || word.equals("「")) {
						start = s;
						boolean find = false;
						for (int e = s + 1; e <= this.getWord(s).sentence.getEndWordIdx(); e++) {
							...
							if (word.equals("\"") || word.equals("''") || word.equals("”") || word.equals("」")) {
								...
								while (backTrace >= 0 && !this.getWord(backTrace).posTag.equalsIgnoreCase("PU")) {
									...
									if (this.getDocument().ontoCommon.getEnDictionary().reportVerb.contains(w.word)
										...
										while(true) {
											...
											if(vv.getLeaves().get(0).leafIdx<=this.getWord(end).indexInSentence) {
												break;
											}
											vv = vv.parent;
										}
								...
								if (VP != null) {
									...
									if (NP == null) {
										for (int m = VP.childIndex + 1; m < VP.parent.children.size(); m++) {
											if (VP.parent.children.get(m).value.equalsIgnoreCase("NP")) {
												NP = VP.parent.children.get(m);
												break;
											}
									...
									if (NP != null) {
										subject = this.getDocument().ontoCommon.formPhrase(NP, reportW
												.getSentence());
//										speaker = subject.source.toLowerCase();
										speaker = this.getWord(subject.end).word.toLowerCase();
//										System.out.println("Speaker: " + speaker + " " + reportW.word + " "
//												+ reportW.getIndex() + " " + reportW.sentence.getSentenceIdx()
//												+ " " + start + " " + this.getWord(start+1).word + " " + end + this.getWord(end-1).word);
									}
								}
								if (speaker.isEmpty()) {
...
//								System.out.println(speaker);
			...
			for(int i=0;i<this.wordCount;i++) {
				...
				if(sentenceId==0) {
					...
					int previousSentenceStartWordIdx = this.sentences.get(sentenceId-1).getStartWordIdx();
					String previousSentenceSpeaker = this.getWord(previousSentenceStartWordIdx).speaker;
					int nextSentenceStartWordIdx = this.sentences.get(sentenceId+1).getStartWordIdx();
					String nextSentenceSpeaker = this.getWord(nextSentenceStartWordIdx).speaker;
					String speaker = word.speaker;
					if(speaker.equals(previousSentenceSpeaker) && speaker.equalsIgnoreCase(nextSentenceSpeaker)) {
						word.toSpeaker.add(word.nextSpeaker);
						word.toSpeaker.add(word.previousSpeaker);
					} else if(!speaker.equals(previousSentenceSpeaker) && speaker.equalsIgnoreCase(nextSentenceSpeaker)) {
						word.toSpeaker.add(word.previousSpeaker);
					} else if(speaker.equals(previousSentenceSpeaker) && !speaker.equalsIgnoreCase(nextSentenceSpeaker)) {
						word.toSpeaker.add(word.nextSpeaker);
					} else {
